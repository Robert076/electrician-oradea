  ...
  try {
    ...
    if (fileBase64 && fileName) {
      ...
      const { error: uploadError } = await supabaseAdmin.storage
        .from("posts")
        .upload(filePath, buffer, { contentType: "image/*" });
    ...
    if (error) {
      console.error("Eroare inserare postare:", error);
      return NextResponse.json({ error: "Eroare la salvarea postării." }, { status: 500 });
    }
